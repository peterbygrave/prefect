...
from prefect.types._datetime import pendulum_instance
...
async def ls(
    ...
    for task_run in sorted(task_runs, key=lambda d: d.created, reverse=True):
        ...
        table.add_row(
            ...
            pendulum_instance(timestamp).diff_for_humans(),
        )
        ...
        while more_logs:
            ...
            for log in reversed(page_logs) if tail and not reverse else page_logs:
                app.console.print(
                    ...
                    (
                        f"{pendulum_instance(log.timestamp).to_datetime_string()}.{log.timestamp.microsecond // 1000:03d} |"
